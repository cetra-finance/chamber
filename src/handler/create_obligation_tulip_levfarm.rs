...
pub struct CreateObligationTulipLevfarmAccounts<'c, 'info> {
...
#[inline(always)]
pub fn create_obligation_tulip_levfarm<'c, 'info>(
    accounts: Box<CreateObligationTulipLevfarmAccounts<'c, 'info>>,
) -> Result<()> {
    ...
    // 3. Create position obligation
    {
        let ix = create_user_farm_obligation(CreateUserFarmObligation {
